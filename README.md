...
- Аутентификация JWT (RSA-256) с поддержкой [Google OAuth2](docs/google-auth-integration.md)
...
[Сервис доставки уведомлений](docs/email-notification-service.md)
...
- События (регистрация пользователя) сохраняются в таблице `outbox` в рамках одной транзакции с бизнес-данными
- Фоновый процесс (`@Scheduled`) считывает новые события и отправляет их в Kafka
- После успешной отправки статус события обновляется (`NEW → SENT`)
- В случае ошибки событие помечается как `FAILED` с возможностью повторной обработки

Дополнительно:

- Отправка событий выполняется асинхронно через выделенный `ThreadPoolTaskExecutor`
- Используются callbacks (`onSuccess / onFailure`) для обработки результата отправки
